...
本文件是 `remotion-video` 工作流中的“场景规划与实现阶段”参考协议，由主 Agent 指派 SubAgent 读取并执行。

## 输入契约

- `skillRoot`: `remotion-video` skill 的绝对路径
...
4. `{skillRoot}/rules/` 目录下的 Remotion 最佳实践规则（按需读取相关文件）

如果当前场景涉及动画编排、文本动画、时序控制、字幕、音频、资源加载、Composition 配置等 Remotion 常见问题，必须继续按需读取 `{skillRoot}/rules/` 下的相关规则文件（如 `animations.md`、`text-animations.md`、`timing.md`、`subtitles.md`、`audio.md`、`assets.md`、`compositions.md` 等）。

**强制要求**：

- Remotion 最佳实践规则位于当前 skill 内部的 `{skillRoot}/rules/` 目录
- 按场景需要读取对应的规则文件，不要一次性全部读取
...
- [ ] 按需读取 `{skillRoot}/rules/` 下的 Remotion 规则文件
- [ ] 生成 `{planPath}`
- [ ] 执行 scene-plan 校验
